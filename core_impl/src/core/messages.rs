...
#[derive(DdsType, Debug)]
pub struct ProviderRequest<T: TypeSupport + XTypesSerialize + for <'a> XTypesDeserialize<'a>>  {
    pub id: u32,
    pub payload: T,
}

impl<T: TypeSupport + XTypesSerialize + for <'a> XTypesDeserialize<'a>> ProviderRequest<T> {
